...
8. **The view reads live records, never a summary of them.** For each contract: what it covers in one line, the three fields the coverage is actually decided by, when it lapses, when it was last used — read from the `activity` records that cite its id — and the one line that ends it. A lapsed contract is shown as lapsed rather than left out, and a superseded one is shown only where the owner asked for the history.
9. **Suggest, and stop there.** Where the `activity log` carries five or more approvals of the same shape — same capability, same skill, same object — inside the last thirty days, say so: the count, the shape, the contract that would have covered them, and the sentence the owner would say to write it. A suggestion is a sentence in a reply and nothing else: no record is drafted anywhere, no preview is treated as accepted, and a suggestion that is ignored is not raised again in the same turn (M5).
...
| Turning a suggestion into a draft record "ready to confirm" | A drafted record is a contract with one step left, and the count that produced it was the agent's, not the owner's decision | Say the count, the shape, and the sentence that would write it — and stop |
