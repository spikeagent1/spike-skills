---
name: briefing
description: "Use when the owner wants the day compiled and cited, read-only: a morning briefing, 'what's happening today', 'what changed overnight', the whole picture across meetings, mail and what's due, or a summary of what the notes already say. Not for changing a task list (daily-task-manager)."
metadata:
  spike-os:
    version: 2.0.0
...
3. Read through the datastore verbs [contracts/datastore.md](../../contracts/datastore.md) defines — `read`, `search`, `list`, `timeline` — and give `timeline` an **explicit range** every time. "Since the last run" is not a range: no read verb carries a position, and asking for one asks for a move rather than a read.
