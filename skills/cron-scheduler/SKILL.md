...
6. Key it four ways, never one, and print all four schemes as part of the definition (M3):
   ...
   - a **per-item key** for every external object the job's action touches — one per item, per message, per record it acts on, built from that object's own stable id — so that a retried firing acting on the same item is one effect and not two, and a partial firing resumes at the item it stopped on rather than at the start;
   - a **delivery key** for the message about the firing, shaped as [contracts/notifications.md](../../contracts/notifications.md) defines.

   These four are independent: a firing that ran and a message that was held are different facts under different keys, and the per-item key is the one that stops a five-minute cadence from acting twice on the same object. The job's resume point is written atomically and only after the item it covers is terminally verified, so the window between acting on an item and recording that it acted never duplicates one or drops one; a retry resumes incomplete work and repeats no verified change and no verified delivery.
...
resume        : output path <path|none> · resume point <written atomically after each item is terminally verified>
keys          : definition <hash of the normalized definition> · occurrence <job-key>@<scheduled-instant> · per item <object stable id, one per item the firing touches> · delivery <as the notification contract shapes it>
