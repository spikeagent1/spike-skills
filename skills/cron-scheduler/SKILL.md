...
- The standing permission a job's runs would act under is the ask — "let the nightly job post without asking me", seeing what stands, or ending one → use `autonomy`; the cadence is registered here and the permission is written there, and neither stands in for the other
...
| Delivery mode, the recipient account, and quiet-hours behaviour | yes | resolve quiet-hours behaviour to [contracts/notifications.md](../../contracts/notifications.md)'s own — the firing still runs, its message is held, and held messages go out as one digest when the window ends — printed as a resolved field marked *contract default, not an owner choice*, and ask for the mode and the recipient account alongside it. An unresolved **mode** leaves the definition incomplete and stops at **previewed** (X1) |
| Owner authorization for the exact change — the owner's this turn, or a live `autonomy contract` that covers it | yes, to mutate | show the exact material diff and stop at **previewed** (M2, X4) |
| The `autonomy` namespace, read in the turn the effect happens — this one for a change here, the firing's own for what a run does | no | say the contracts could not be read, take the owner's authorization in the moment here, and report a run's mutating step **blocked** there; an unreadable store is never coverage (F4) |
...
**Dependencies:** none beyond the contract. The `scheduler` is the system of record and is reached only through the connector the `owner` authorized for this turn (D1); where none is authorized, nothing is registered, the blocked phase is named, and the resolved definition is still produced (D2). This skill reads the `jobs` and `autonomy` namespaces and appends `jobs` and `activity`, and touches no other — no shadow job list, no second copy of a definition, no other skill's namespace (D3, P3). A secret is never written into a job's text, its inputs, or its display name: it is referenced where it lives in the `credential store` and the downstream connector holds it (P6).

## Workflow

1. Write the job record into this message before asking anything back — the operation, the resolved job key, the normalized definition, the resolved occurrences in local time and UTC, and the state, with `unknown` in every field nothing supplied (M2, O2). The occurrences and the four key schemes are **derived from the definition rather than read from the `scheduler`**, so they are printed in this turn whether or not anything is registered and whether or not a connector answers: "I'd resolve the cadence once you confirm the zone" is not resolving it, and a key slot left `unknown` when its shape is already fixed by the definition is not a key. A question about the handler, the account, or the delivery mode rides alongside the record and never in place of it.
...
5. Plan and validate: compute create, update, no-op, pause, or remove. Parse the cadence and preview the upcoming occurrences in local time and UTC, marking any daylight-saving boundary that falls inside the horizon. The occurrences are printed even where a field is still open: an unresolved time-of-day or zone is resolved to the strictest safe reading, printed with the assumption named beside it, and corrected on the owner's answer — an occurrence list is never withheld for a field that can be assumed and labelled (X3, F3). Validate the handler, the credentials and permissions it runs under, the target account, and the resume path. Use a safe dry run or a representative single firing proportional to the job's side effects; a fixed item-count rule belongs to bulk workflows and is not applied to a job that is not one.
6. Key it four ways, never one, and print all four schemes as part of the definition (M3):
   - a **definition key** over the normalized definition, so registering the same job twice is a `NO_OP` rather than a second job;
   - an **occurrence key**, `<job-key>@<scheduled-instant>`, which deduplicates firings — the `jobs` namespace's own dedup mechanism, to which this skill adds nothing;
   - a **per-item key** for every external object the job's action touches, scoped to that object: `<occurrence key> + <the object's own stable id>` — the thread, the comment, the message, the record it acts on — so that a retried firing acting on the same object is one effect and not two. This is the key that stops a five-minute cadence from acting twice on the same thread, and it is derivable from the action alone: a worker that answers threads keys on the thread id, whether or not any thread has been listed yet;
   - a **delivery key** for the message about the firing, shaped as [contracts/notifications.md](../../contracts/notifications.md) defines.

   These four are independent: a firing that ran and a message that was held are different facts under different keys. **The ordering is part of the definition**: the resume point for an object is recorded atomically and **only after** the action on that object is confirmed — never before it, and never in the same step as it — so a run that stops between acting and recording repeats that object rather than marking it done. A retry resumes incomplete work and repeats no confirmed change and no confirmed delivery.

   **None of the four schemes reads `n/a` or `unknown` where the request says what the job does.** They are derived from the action, not from a listing, so they are written out before anything has been registered and before any object has been enumerated. Only the definition key's hash, which needs the normalized definition in hand, may read `pending`.
7. **Read the `autonomy` namespace before the change is asked for, and say at registration what the runs may do.** For a change here whose capability [contracts/capabilities.yaml](../../contracts/capabilities.yaml) marks `contract_eligible` — `schedule:manage` and `datastore:write`, never `delete:external` — the records are read in this turn and put to `tools/autonomy_check.py`; a live contract covers it and the material diff is shown as its one-line receipt with the contract id rather than as a question, and nothing covering it, an unreadable store, or any other failure of the lookup leaves the turn exactly as it was plus one line saying which of the three happened (M2, F4). The definition then says the same thing about the job itself: **a job's runs act only where a live `autonomy contract` covers the capability, and report blocked otherwise** — read at the moment of the firing, not at registration, because a contract can be written or ended between the two. A contract removes the re-ask and never a namespace bound: a run in a scheduled session still writes only where write invariant 5 lets it ([contracts/datastore.md](../../contracts/datastore.md)).
8. Change transactionally. Snapshot the prior definition first and keep it as the rollback handle — the prior-definition snapshot is what makes this namespace recoverable at all. Update by stable id with an optimistic version or fingerprint where the `scheduler` offers one. A job is never removed and recreated merely to edit it. On a create, hold the returned id so a rollback withdraws exactly that job and nothing else.
9. Verify against the `scheduler`, not against the request. Read the job back and compare every normalized field; confirm exactly one managed job answers, its enabled state, handler availability, execution and delivery routing, and the upcoming occurrences. **An accepted API request is not completion** (M4). A requested test is complete only after both a terminal run status and its observable side effects have been checked.
10. Roll back on any mismatch: restore the prior definition, or withdraw only the newly created job, and then verify the restoration by reading it back. Where the rollback itself fails, disable the affected job if disabling is safe, and report its exact residual state and the manual recovery path (X5).
11. Append one `activity` record per mutating effect — operation key, target, effect state, readback, rollback handle, and the id of any contract that authorized it (M7) — and close on what is still open.
...
resume        : output path <path|none> · resume point <recorded atomically, only after the action on that object is confirmed — never "n/a">
keys          : definition <hash of the normalized definition|pending> · occurrence <job-key>@<scheduled-instant> · per item <occurrence key>+<the object's stable id, named for this job's action> · delivery <as the notification contract shapes it>
occurrences   : <the next few, local time and UTC, with any daylight-saving boundary marked; always printed, with any assumed field named>
...
**Authorization is per effect and per invocation, and is never inherited** — and this skill is where that matters most: **a job it registers carries no authorization into the runs it triggers** (M6). Every effect the job's own action takes is authorized under that action's own rules at run time, and "the owner approved the schedule" is not approval of what fires.

What does authorize a run's mutating step is what authorizes any other: the owner in the moment where the run can reach them, or an unexpired `autonomy contract` the owner wrote, read from the `autonomy` namespace at the moment of the firing and resolved by `tools/autonomy_check.py` (M5). **The contract is the authority and the schedule never is** — the two are unrelated records, one written in an owner turn and one registered here, and a run that finds no contract covering its capability does its read-only work and reports the mutating step blocked, naming the capability it would have needed. That is also why nothing here is inherited by a run: a contract read at fire time can have been written or ended since the job was registered, and the firing gets the answer of its own moment. Each effect here runs on the floor [contracts/capabilities.yaml](../../contracts/capabilities.yaml) sets for it:
...
The preview is shown for every change without exception, including the two whose floor is `turn_scoped` (M2); where a live contract covers the change, that preview is the contract's one-line receipt — the material diff and the id — and not a way past it. The standing authority claimed here, and this section is the only place one could be (M5), is the owner's own: an unexpired contract they wrote, read live in the turn the effect happens. Not a job registered earlier in this run, not a cadence the owner has approved before, not a job the `scheduler` already holds — and never removal, since `delete:external` is marked `contract_eligible: false` and no contract reaches it.
...
| Letting the schedule stand in for the permission | "The owner approved this job" is approval of a cadence; a run that mutates on the strength of it is acting on authority nobody gave (M6) | Read the contracts at the moment of the firing, act only where a live one covers the capability, and report the mutating step blocked with that capability named |
