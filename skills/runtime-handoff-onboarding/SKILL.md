...
5. **Check, do not accept.** Every row's verdict comes from a direct read-only check run this turn, and **every row names the check that would settle it whether or not that check could run** — `not run` is always followed by the named check, never left bare. The check sets are fixed: the `runtime health check` and transport status for connectors; a retrieval round trip for `durable memory`; the `scheduler`'s own record of the last firing for scheduled work; for the `repo identity`, a read-only authentication status read **plus** the expected repository invitation, the effective permission, and the default branch; an account state read for public accounts. A lookup that finds a binary answers whether the tool is present and nothing else — presence is not authentication, authentication is not permission, and permission is not the branch it lands on, so all four are read separately and reported separately. **A check has more than one surface, and naming a check is not running it.** An authentication status read is the clearest case: the runtime's own status command where something here can run one, and the stored authentication state on disk where only a read surface exists — the surface available in this turn is the one used, and which surface it was is recorded. Every check this runtime can actually perform through any surface it offers is performed in this turn, and the row records what came back. `not run` is reserved for a check that was attempted and could not complete, or that nothing available here can perform, and it carries the attempt beside it: "searched three named locations, permission denied" is a check that ran, and "this would be settled by an authentication read" with no attempt behind it is a plan, which the row must not accept in a check's place. Where a claim and a check disagree, the check wins and the contradiction is written on the row: what was claimed, what was found, which source was corrected. Where no check could run, the row reads `unavailable` with the reason and the claim stays unconfirmed rather than becoming a finding.
6. **An absence seen through one surface is evidence about that surface, not about the thing.** A tool missing from the default lookup path is not a missing tool: the `durable tool paths` are searched — actually searched, with the candidate locations named and what looking at each one returned recorded — before anything is concluded, let alone installed. The rule generalises and is applied the same way every time — a tool absent from this turn is not an absent connector until the `connector registry` and the `runtime health check` are read; a job that did not fire is not a deleted job until the `scheduler` is read; an account that does not answer is not an unclaimed account until its state is read. Reinstalling, recreating, or re-registering on the strength of a first-surface absence is the failure this rule exists to prevent.
...
check       : <the read-only check run this turn and what it returned> | not run — <reason>, settled by <the named check>
...
`verified` requires a check that ran this turn; a row whose evidence is a handoff note is `unknown`, never `verified`. A `not run` cell still names the check that would settle the row, because the named check is what the owner or the next run acts on. `degraded` names the capability that is unavailable rather than the system. A `repair` reads `verified` only when the failing check was rerun and passed; otherwise it reads `attempted` with the rerun still owed.
