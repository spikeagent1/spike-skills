...
3. **Read the boundaries before the brief.** The `owner`'s disclosure boundaries and authority rules live in `profile` and are read there rather than recalled, over an explicit timeline range named in the run — first and last date — never "since the last run" or "recently", so what was and was not read is legible; a record whose compiled state is older than its newest timeline entry is context and never the current boundary (F2, P1, P2). A `search` hit is `read` in full before it is used.
