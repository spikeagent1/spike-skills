...
1. **Produce the action in this turn.** The routing decision, the drafted reply or entry at full length, the triage table with one disposition per message, the facilitator artifacts, the consent question where one is owed, and the verification each mutation would need all appear in this message, at the furthest state the inputs and the authorization actually reach. A missing input empties its field, never the run: "tell me the account and I will reply" is not the reply, and a description of how the mail would be handled is not the triage. Where a phase cannot run, name it as the blocked phase with what would unblock it, and deliver everything upstream (X3, D2). An unreachable connector or an unauthorized account blocks the send — and nothing before it: the disposition of every message, the drafted answer, the roster diff, the broadcast list, and the owner line are all produced anyway, as exact text. **A tool environment with no connector at all blocks every read and every mutation and nothing else**: the obligations, the drafts, the triage, and the report are then written from the request's own contents, with each unsupplied fact carried as a visibly marked slot. A slot stands in for a fact nobody supplied — a name, a date, a link, a figure — never for the answer itself: the reply to an ordinary question is written out at full length from what the message already asks, and revised when the fact arrives. A response whose substantive fields are all slots is a deferral, not an answer (X6). Where the request names classes of work rather than items — "answer replies, join discussions, follow people" — the triage table is still rendered here, one row per class in the obligation order the next step fixes, each row carrying its own slot: the order worked is itself part of the deliverable, and a promise to triage later is not a triage. "No inbox to read" is a reason the send is `PREVIEWED`, never a reason the turn produces a status block instead of the work.
...
4. **A requested entry, public writing, or a belief review** — read [references/writing-and-belief.md](references/writing-and-belief.md). **The entry is drafted here, in full, in this turn**: two or three short paragraphs in the `agent`'s own first person, opening on the concrete trigger the request itself names — what the `owner` shared, and the specific thing about it that stuck — with any fact nobody supplied carried as a visibly marked slot rather than invented or used as a reason to withhold the draft. A template with X and Y in it is not a draft, and a status block saying the draft is blocked is not a draft. That finished draft is then what goes to `public-post-workshop` for its independent cold review and its unmerged pull request. **The rubric pass still runs here, in the same turn, on the draft just written** — audience fit, truthful trigger, voice, attribution, factual grounding, disclosure, economy — reported dimension by dimension with the concrete weaknesses it found, and labelled a **self-check**: the writer already knows what the draft is trying to say, which is exactly the knowledge the cold review is meant to lack, so a self-check is never recorded as PASS and never opens a pull request. State in the same breath that the entry stays an **unpublished draft** until the independent fresh reviewer has run, and never claim a publication, a landing, or a review verdict before then (M8, X3). The belief question is asked after that gate, never before it.
...
roster       : <diff row> · view <columns shown, owner email omitted> · broadcast <recipients> · excluded <the agent's own address, by name>
...
self-check   : <dimension> -> <finding>   (a writing task always renders one; never PASS, never a review verdict)
draft state  : unpublished draft — never published or landed until the independent fresh reviewer runs
...
| Naming `public-post-workshop`'s gate instead of running any review at all | Handing the gate its name is not handing it a reviewed draft, and the requester is left with neither a verdict nor a list of what is weak | Run the rubric pass on the draft in this turn, label it a self-check, and name the independent fresh reviewer as the outstanding gate |
| Reporting the whole turn blocked because no connector answered | A tool environment with no connector bounds the reads and the sends, and nothing about the drafting, the triage, or the report | Produce the work from the request's own contents with each unsupplied fact marked, and stop at `PREVIEWED` |
