...
| The source artifact and the version of it being released | yes | ask once, in the same turn as a release record built on the strictest safe reading — encrypted local render, no destination — with every unresolved field written `unknown` (X1, X3) |
| Destination account, object path, visibility, audience, expiry | yes, to release | stop at `PREVIEWED` with the record rendered; a destination, an account, or an audience nobody named is never inferred from the artifact, from an earlier release, or from what the content is about (X3) |
...
| Review status of the content, and the rollback or correction path | yes, to release | stop at `PREVIEWED` and name which of the two is missing (X1) |
...
The release record is in this message and is not promised for the next one: describing what would be checked, offering to preview once the destination is known, or asking for the audience before rendering anything is a failure to deliver it. In order: any data-quality warning that changes the decision — an unauthorized destination, a redaction that changed what the artifact says, a readback that could not be taken (O1); the release record with `unknown` and `pending` in place; the exact preview of the mutation; the state; the verification evidence; the retry and rollback handles; and what is still open. The password appears in none of it (P6).
