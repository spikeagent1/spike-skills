...
- **P2** Never fill a gap from memory; mark the section unavailable instead (`skills/briefing/SKILL.md:96`).
- **P3** Touch only namespaces declared in `reads_from` and `writes_to` (`contracts/datastore.md`).
- **P4** Store and show minimum sensitive detail: concise attributed records, never raw transcripts (`skills/owner-context-onboarding/SKILL.md:98`).
...
- **S1** Make no professional medical, legal, financial, structural, electrical, gas, or fire-safety determination (`skills/fitness-coach/SKILL.md:32`).
