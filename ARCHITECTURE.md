...
are reserved, and may be named and read about, never written: `calendar/` and
`inbox/`, until a provider conduit exists. `autonomy/` holds the owner's standing
permissions, written by the `autonomy` manager skill alone — an autonomy contract
is written in an interactive owner turn, never on a skill's initiative, and
honored in any session kind.
