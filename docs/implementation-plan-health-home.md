...
Release the first everyday-life cohort after audience/community by adding portable health and home/lifestyle skills plus repository gates that keep every skill independently installable, attributable, dependency-declared, privacy-safe, and covered by synthetic evaluations.
...
Minimum complete change: add the listed health and home/lifestyle skills, add synthetic evals, update catalogs/docs, and add deterministic validation for manifests, evals, catalogs, local-state ignore rules, and obvious secret/private-data leaks.

Complexity decision: the file count is high because a library release is many independent packages. The architecture stays small: no services, database, package manager, shared storage, or new runtime. Complete option selected in spawned-session mode.
...
Issue 1 accepted: prose gates are not enough. Recommendation selected: add `tools/validate_repo.py` and `make validate`.
...
Issue 2 accepted: catalog updates must be checked deterministically. Recommendation selected: validate approved catalog entries, domain released entries, and skill directories together.
...
[+] tools/validate_repo.py                             [+] Skill release gate
  ...
  |   |-- catalog approved entry exists                  |-- [EVAL] emergency request escalates
  |   |-- domain release entry exists                    |-- [EVAL] medical diagnosis refused
  |   |-- dependency/provenance declared                  |
  |-- parse_domains()/parse_approved()                 [+] Home/lifestyle invocation
...
- Catalog claims a missing release: covered by `tools/validate_repo.py`.
- Private local state committed: covered by `.gitignore` and tracked-file scan.

## Parallelization

Lane A: health skills -> catalog update. Lane B: home/lifestyle skills -> catalog update. Lane C: validator/schema -> Make target. Lane D: docs/report after A+B+C. Conflict flag: A and B both touch `catalog/`.

## Implementation Tasks

- [x] T1 (P1) health skills with boundaries and evals.
- [x] T2 (P1) home/lifestyle skills with source discipline and evals.
- [x] T3 (P1) deterministic validation gate.
- [x] T4 (P2) docs/catalog/release evidence.
...
- **VERDICT:** CEO + ENG CLEARED - ready to implement and ship as an unmerged PR.
